...
class StudentGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Student entity")
        ...
        .repeat(2) {
            ...
            .post("/api/students")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "sid":"SAMPLE_TEXT", "lastName":"SAMPLE_TEXT", "firstName":"SAMPLE_TEXT", "email":"SAMPLE_TEXT", "phone":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_student_url")))
            .pause(10)
            .repeat(5) {
